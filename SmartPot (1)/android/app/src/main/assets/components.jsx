...
// Brand wordmark (transparent PNG). `mono` renders it white for the green header;
// otherwise the natural green/brown art shows on light or dark surfaces.
function Logo({ height = 26, mono = false }) {
  return (
    <img src="assets/smartpot-logo.png" alt="SmartPot" style={{
      height, display: 'block', width: 'auto',
...
function ConnChip({ conn }) {
  ...
  const map = {
    ...
    offline: { c: t.bad, label: 'Device offline', icon: 'cloudoff' },
...
function PumpControl({ mode, onWater, watering, secondsLeft }) {
  ...
  const subtitle = watering
    ? `Dispensing · ${Number(secondsLeft).toFixed(1)}s left`
...
Object.assign(window, {
  Logo, ConnChip, Ring, SensorTile, AreaChart, smoothPath, Segmented, Switch, PumpControl, AlertRow, Toast,
});
