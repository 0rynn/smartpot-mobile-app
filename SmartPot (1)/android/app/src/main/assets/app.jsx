...
// Built from the same status helpers that drive the UI, so alerts always match
// the colors/labels. Each metric can fail LOW or HIGH.
function deriveConditions(latest, t, pumpMode) {
  if (!latest) return [];
  const out = [];
  const moist = latest.soil_moisture, lux = latest.light_lux, temp = latest.temperature;

  // Soil moisture — Healthy 30–80%, Warning 15–30 / 80–90, Critical <15 / >90
  if (moist != null) {
    if (moist < 15) out.push({ key: 'soil-crit-low', sev: 'bad', title: 'Soil critically dry', body: `Moisture at ${Math.round(moist)}%. ${pumpMode === 'auto' ? 'Auto-watering should engage — check the reservoir.' : 'Water now or switch on the pump.'}` });
    else if (moist < 30) out.push({ key: 'soil-low', sev: 'warn', title: 'Soil is getting dry', body: `Moisture at ${Math.round(moist)}%. ${pumpMode === 'auto' ? 'Auto-watering engages at 30%.' : 'Consider watering soon.'}` });
    else if (moist > 90) out.push({ key: 'soil-crit-high', sev: 'bad', title: 'Soil waterlogged', body: `Moisture at ${Math.round(moist)}%. Stop watering and let it drain to avoid root rot.` });
    else if (moist > 80) out.push({ key: 'soil-high', sev: 'warn', title: 'Soil very wet', body: `Moisture at ${Math.round(moist)}%. Hold off on watering.` });
  }

  // Light — Healthy 4,000–20,000 lux, Warning 1,000–4,000 / 20,000–25,000, Critical <1,000 / >25,000
  if (lux != null) {
    if (lux < 1000) out.push({ key: 'light-crit-low', sev: 'bad', title: 'Far too dark', body: `Only ${Math.round(lux).toLocaleString()} lux. Move the plant to a much brighter spot.` });
    else if (lux < 4000) out.push({ key: 'light-low', sev: 'warn', title: 'Low light', body: `${Math.round(lux).toLocaleString()} lux is below the healthy range. A brighter spot would help.` });
    else if (lux > 25000) out.push({ key: 'light-crit-high', sev: 'bad', title: 'Light too intense', body: `${Math.round(lux).toLocaleString()} lux can scorch leaves. Add shade or move it back.` });
    else if (lux > 20000) out.push({ key: 'light-high', sev: 'warn', title: 'Very bright', body: `${Math.round(lux).toLocaleString()} lux is near the upper limit. Watch for leaf scorch.` });
  }

  // Temperature — Healthy 18–29°C, Warning 10–18 / 29–35, Critical <10 / >35
  if (temp != null) {
    if (temp < 10) out.push({ key: 'temp-crit-low', sev: 'bad', title: 'Temperature too low', body: `${temp.toFixed(1)}°C risks cold damage. Move somewhere warmer.` });
    else if (temp < 18) out.push({ key: 'temp-low', sev: 'warn', title: 'Cooler than ideal', body: `${temp.toFixed(1)}°C is below the healthy 18–29°C band.` });
    else if (temp > 35) out.push({ key: 'temp-crit-high', sev: 'bad', title: 'Temperature too high', body: `${temp.toFixed(1)}°C risks heat stress. Cool it down or add shade.` });
    else if (temp > 29) out.push({ key: 'temp-high', sev: 'warn', title: 'Warmer than ideal', body: `${temp.toFixed(1)}°C is above the healthy 18–29°C band.` });
  }
  return out;
}
...
function SmartPotApp() {
  ...
  const { latest, history, conn, lastUpdate } = useSmartPotData(range);
  ...
  useEffect(() => {
    localStorage.setItem('sp_pumpmode', pumpMode);
    setPumpModeRemote(pumpMode); // tell the ESP32 which mode to run in
  }, [pumpMode]);
  ...
  // Watering — sends a real command to the ESP32 pump via Firebase.
  const onWater = useCallback(async () => {
    if (watering) return;
    const dur = 2.7; // pump runs for 2.7s per activation
    setWatering(true); setSecondsLeft(dur);
    const res = await sendPumpCommand(true, dur);
    if (!res.ok) {
      // couldn't reach the device — abort the countdown and tell the user
      setWatering(false); setSecondsLeft(0);
      setToast({ key: 'pump-err', sev: 'bad', title: 'Pump command failed', body: res.reason || 'Could not reach SmartPot.' });
      clearTimeout(toastTimer.current); toastTimer.current = setTimeout(() => setToast(null), 3600);
      return;
    }
    ...
    const iv = setInterval(() => {
      ...
      setSecondsLeft(left);
      if (left <= 0) {
        clearInterval(iv); setWatering(false);
        sendPumpCommand(false, 0); // stop command (ESP32 also auto-stops after duration)
        setToast({ key: 'watered', sev: 'good', title: 'Watering complete', body: 'Pump ran for 2.7s.' });
        clearTimeout(toastTimer.current); toastTimer.current = setTimeout(() => setToast(null), 3200);
      }
    }, 100);
  }, [watering]);

  // Automatic mode: turn the pump on at ~30% soil moisture, off at ~70%.
  useEffect(() => {
    if (pumpMode !== 'auto' || !latest || watering) return;
    const moist = latest.soil_moisture;
    if (moist != null && moist <= 30 && Date.now() - lastAutoRef.current > 12000) {
      lastAutoRef.current = Date.now();
      setToast({ key: 'autowater', sev: 'info', title: 'Auto-watering', body: `Soil at ${Math.round(moist)}% — running pump until ~70%.` });
      clearTimeout(toastTimer.current);
      toastTimer.current = setTimeout(() => setToast(null), 3500);
      onWater();
    }
...
// Embed mode: when running inside the native Android WebView wrapper (or with
// ?embed=1), render the app full-bleed — no fake device bezel on a real device.
const SP_EMBEDDED = (() => {
  try {
    if (new URLSearchParams(location.search).get('embed') === '1') return true;
    if (/Android.*; wv\)/.test(navigator.userAgent)) return true;
  } catch (e) {}
  return false;
})();
...
function Root() {
  ...
  if (SP_EMBEDDED) {
    return (
      <div style={{ height: '100dvh', width: '100%', overflow: 'hidden', background: dark ? '#0d1411' : '#eef3ef' }}>
        <SmartPotApp />
      </div>
    );
  }
