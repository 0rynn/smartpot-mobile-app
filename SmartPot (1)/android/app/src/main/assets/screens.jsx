...
// Scores follow the documented bands: 100 inside the healthy range, tapering
// through the warning band, falling to 0 past the critical edges. Low and high
// failure modes are symmetric (too much is as harmful as too little).
const clamp01 = (x) => Math.max(0, Math.min(100, x));
function scoreTemp(c) {
  if (c == null) return 70;
  if (c >= 18 && c <= 29) return 100;
  if (c < 18) return c <= 10 ? 0 : 55 + ((c - 10) / 8) * 45;
  return c >= 35 ? 0 : 55 + ((35 - c) / 6) * 45;
}
function scoreMoist(m) {
  if (m == null) return 70;
  if (m >= 30 && m <= 80) return 100;
  if (m < 30) return m <= 15 ? 0 : 55 + ((m - 15) / 15) * 45;
  return m >= 90 ? 0 : 55 + ((90 - m) / 10) * 45;
}
function scoreLight(lux) {
  if (lux == null) return 70;
  if (lux >= 4000 && lux <= 20000) return 100;
  if (lux < 4000) return lux <= 1000 ? 0 : 55 + ((lux - 1000) / 3000) * 45;
  return lux >= 25000 ? 0 : 55 + ((25000 - lux) / 5000) * 45;
}
