...
// ── Status helpers ───────────────────────────────────────────────────
// Every metric has BOTH a low and a high failure mode — status is distance from
// the ideal band, never "higher = better". Three levels: good / warn / bad.

// Soil moisture: Healthy 30–80% · Warning 15–30% or 80–90% · Critical <15% or >90%
function waterStatus(p, t) {
  if (p == null) return { label: '—', color: t.textMuted, level: 'ok' };
  if (p < 15) return { label: 'Critically Dry', color: t.bad, level: 'bad' };
  if (p < 30) return { label: 'Low', color: t.warn, level: 'warn' };
  if (p <= 80) return { label: 'Healthy', color: t.good, level: 'good' };
  if (p <= 90) return { label: 'Saturated', color: t.warn, level: 'warn' };
  return { label: 'Waterlogged', color: t.bad, level: 'bad' };
}
// Light: Healthy 4,000–20,000 lux · Warning 1,000–4,000 or 20,000–25,000 · Critical <1,000 or >25,000
function lightStatus(lux, t) {
  if (lux == null) return { label: '—', color: t.textMuted, level: 'ok' };
  if (lux < 1000) return { label: 'Too Dark', color: t.bad, level: 'bad' };
  if (lux < 4000) return { label: 'Low Light', color: t.warn, level: 'warn' };
  if (lux <= 20000) return { label: 'Healthy', color: t.good, level: 'good' };
  if (lux <= 25000) return { label: 'Very Bright', color: t.warn, level: 'warn' };
  return { label: 'Too Bright', color: t.bad, level: 'bad' };
}
// Temperature: Healthy 18–29°C · Warning 10–18 or 29–35 · Critical <10 or >35
function tempStatus(c, t) {
  if (c == null) return { label: '—', color: t.textMuted, level: 'ok' };
  if (c < 10) return { label: 'Too Cold', color: t.bad, level: 'bad' };
  if (c < 18) return { label: 'Cool', color: t.warn, level: 'warn' };
  if (c <= 29) return { label: 'Healthy', color: t.good, level: 'good' };
  if (c <= 35) return { label: 'Warm', color: t.warn, level: 'warn' };
  return { label: 'Too Hot', color: t.bad, level: 'bad' };
}
