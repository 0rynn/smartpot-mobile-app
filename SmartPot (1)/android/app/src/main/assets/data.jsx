...
// "Outside threshold" = outside the documented healthy bands (low OR high).
function sensorsOutsideThreshold(latest) {
  if (!latest) return false;
  const m = latest.soil_moisture, tp = latest.temperature, l = latest.light_lux;
  const moistBad = m != null && (m < 30 || m > 80);
  const tempBad = tp != null && (tp < 18 || tp > 29);
  const lightBad = l != null && (l < 4000 || l > 20000);
  return moistBad || tempBad || lightBad;
}
