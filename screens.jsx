...
function HomeScreen({ latest, conn, lastUpdate, overall, onWater, watering, secondsLeft, pumpMode }) {
  ...
  if (!latest) {
    return (
      <div style={{ padding: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 14 }}>
  ...
  return (
    <div style={{ padding: '16px 16px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ padding: '4px 2px', display: 'flex', flexDirection: 'column', gap: 9, alignItems: 'flex-start' }}>
        <Logo height={34} plate plateBg="#ffffff" />
        <div style={{ fontSize: 13, color: t.textSec, fontWeight: 600, paddingLeft: 2 }}>
          Version 1.0.0 · {conn === 'live' ? 'Connected' : 'Demo mode'}
        </div>
      </div>
      ...
      <div>
        ...
        <Card>
          <SettingsRow icon="moon" accent={t.primary} accentSoft={t.primarySoft} title="Dark Mode" subtitle="Easier on the eyes at night"
            right={<Switch on={dark} onChange={setDark} />} />
          <Divider />
          <SettingsRow icon="bell" accent={t.warn} accentSoft={t.lightSoft} title="Alerts &amp; Notifications" subtitle="Warn me about plant conditions"
          ...
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 15 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12.5, color: t.textSec, marginTop: 1 }}>
                  {pumpMode === 'auto' ? 'Waters automatically when soil is dry' : 'You start watering manually from Home'}
                </div>
              </div>
            </div>
        ...
        <Card>
          {team.map((m, i) => (
            <React.Fragment key={m}>
              <SettingsRow icon="users" accent={t.primary} accentSoft={t.primarySoft} title={m} />
