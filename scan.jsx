...
function ScanScreen({ latest, pushToast }) {
  ...
  const onSave = async () => {
    if (!result) return;
    setSaving(true);
    // Best-effort cloud write (uses the documented schema). In demo mode or when
    // DB rules block the write this fails silently — the scan still saves locally.
    ...
    setSaved(true);
    pushToast(r.ok && r.reason !== 'demo'
      ? { key: 'scan-saved', sev: 'good', title: 'Saved to SmartPot', body: `${result.badge} · synced to cloud` }
      : { key: 'scan-saved', sev: 'good', title: 'Scan saved', body: `${result.badge} · ${(result.confidence * 100).toFixed(0)}% confidence` });
  };
